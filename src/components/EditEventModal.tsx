...
import {
  normalizeTagName,
  syncTagIdentitiesFromEventFields,
...
export default function EditEventModal({
  ...
  const [showType, setShowType] = useState<ShowType>(() =>
    normalizeShowType(event.show_type),
  );
  ...
  const [producers, setProducers] = useState<string[]>(() =>
    coalesceTagList(event.producers),
  );
  const [designers, setDesigners] = useState<string[]>(() =>
    coalesceTagList(event.featured_designers),
  );
  const [artists, setArtists] = useState<string[]>(() =>
    coalesceTagList(event.featured_artists),
  );
  const [specialGuests, setSpecialGuests] = useState<string[]>(() =>
    getSpecialGuests(event.custom_tags),
  );
  const [hairMakeup, setHairMakeup] = useState<string[]>(() =>
    coalesceTagList(event.hair_makeup),
  );
  const [headerTags, setHeaderTags] = useState<string[]>(() =>
    effectiveHeaderTags(event),
  );
  const [footerTags, setFooterTags] = useState<string[]>(() =>
    coalesceTagList(event.footer_tags),
  );
  const [customTags, setCustomTags] = useState<Record<string, string[]>>(
    event.custom_tags || {},
  );
  const [inlineCustomTypes, setInlineCustomTypes] = useState<
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Invalid official ticket link",
      );
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const resolvedDesigners =
        showType === "fashion" ? resolveTags(clean(designers)) : [];
      const resolvedArtists =
        showType === "music" ? resolveTags(clean(artists)) : [];
      ...
      const resolvedSpecialGuests =
        showType === "music" ? resolveTags(clean(specialGuests)) : [];
      const customTagsWithGuests = withSpecialGuests(
        resolvedCustomTags,
        resolvedSpecialGuests,
      );
      const customMeta = withSpecialGuestsMeta(
        Object.fromEntries(
          inlineCustomTypes.map((t) => [t.slug, { icon: t.icon || "Tag" }]),
        ),
      ...
      const formattedAddress = await resolveVenueFormattedAddress(
        venueVal,
        cityVal,
      );
      ...
      const { data: updatedRows, error: updateError } = await supabase
        ...
        .update({
          ...
          featured_designers: resolvedDesigners.length
            ? resolvedDesigners
            : null,
          ...
          custom_tags: Object.keys(customTagsWithGuests).length
            ? customTagsWithGuests
            : null,
            ...
            {SHOW_TYPE_OPTIONS.map((opt) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "min-h-10 flex-1 rounded-lg border px-3 py-2 font-medium transition-colors type-callout",
                    selected
          ...
          <TagInput
            ...
            expandedExtras={
              <IconPicker
                ...
                onChange={(v) =>
                  setInlineCustomTypes((prev) =>
                    prev.map((t) => (t.slug === slug ? { ...t, icon: v } : t)),
                  )
                }
              />
            }
          ...
          <div className="flex-1">
            ...
            <CustomPerformerCategoryInput
              ...
              onPickExisting={(slug, label) => {
                if (
                  slug === SPECIAL_GUESTS_SLUG ||
                  inlineCustomTypes.some((t) => t.slug === slug)
                )
                  return;
                setInlineCustomTypes((prev) => [
                  ...prev,
