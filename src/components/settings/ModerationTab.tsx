...
import { Button, formErrorClass, formHintClass, menuRowClass, typeCallout } from '../ui';
...
export default function ModerationTab() {
 ...
 if (loading) {
 return <p className={`${typeCallout} text-muted-foreground`}>{t('safety.moderation.loading')}</p>;
 }

 if (reports.length === 0) {
 return <p className={`${typeCallout} text-muted-foreground`}>{t('safety.moderation.empty')}</p>;
 }
 ...
 <div
 key={report.id}
 className={`rounded-lg border border-border bg-muted/30 p-3 ${menuRowClass}`}
 >
 ...
 <p className={formHintClass}>{formatReportTime(report.created_at)}</p>
 ...
 <Link
 to={targetLink.href}
 className={`inline-flex shrink-0 items-center gap-1 ${typeCallout} text-foreground underline-offset-2 hover:underline`}
 >
