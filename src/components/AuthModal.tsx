...
export default function AuthModal({
  ...
  return (
    <Modal onClose={onClose} title={title} panelClassName="max-w-md sm:rounded-xl">
      <div className="p-4 sm:p-6">
        ...
        {mode === 'forgot' ? (
          <p className="mb-4 text-sm text-muted-foreground">{t('auth.forgot.body')}</p>
        ) : null}
        ...
        <form onSubmit={(e) => void handleSubmit(e)} className="space-y-4">
          ...
          {error && !success ? <p className="text-sm text-destructive">{error}</p> : null}
