-- User-created lists of events (e.g. "Greatest shows", model resume)
...
-- user_list_events: only touch if user owns the list
...
CREATE POLICY "Users can insert into own lists"
  ON user_list_events FOR INSERT
  TO authenticated
  WITH CHECK (
    EXISTS (SELECT 1 FROM user_lists WHERE id = list_id AND user_id = auth.uid())
  );
...
CREATE POLICY "Users can delete from own lists"
  ON user_list_events FOR DELETE
  TO authenticated
  USING (
    EXISTS (SELECT 1 FROM user_lists WHERE id = list_id AND user_id = auth.uid())
  );
...
CREATE POLICY "Users can update own list events"
  ON user_list_events FOR UPDATE
  TO authenticated
  USING (
    EXISTS (SELECT 1 FROM user_lists WHERE id = list_id AND user_id = auth.uid())
  )
